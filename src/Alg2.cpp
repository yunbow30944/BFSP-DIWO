...
int calculate_S(int pi_now, int pi_worst, int pi_best, int S_max, int S_min) {
  return (
      int)(((pi_worst - pi_now + epsilon) / (pi_worst - pi_best + epsilon)) *
               (S_max - S_min) +
           S_min);
}
...
int justify_d(int d, const double sigma_min, const double sigma_max) {
  ...
  if (d <= n / 2 && d >= sigma_min) {
    ...
    while (d > static_cast<double>(n / 2) ||
           d < static_cast<double>(sigma_min)) {
...
int generate_d(Sigma &sigma, int i, const double sigma_min,
...
double Sigma::calculate_sigma_k(double sigma_min, double sigma_max) const {
  return (1 - (static_cast<double>(clock() - start)) / t_max) *
             (sigma_max - sigma_min) +
         sigma_min;
}
...
void Sigma::calculate_sigma_i_k() {
  ...
  for (int i = 0; i < pop.size(); i++) {
    if (i < pop.size() / 2) {
      ...
      sigma_i_k.push_back(sigma_k *
                          (1 + 0.5 * ((pop[i][0] - C_median) /
                                      (C_worst - C_median + epsilon))));
    }
  }
}
} // namespace ALG2
...
vector<int> reproduction(const int S_min, const int S_max) {
  ...
  for (const auto &pi_index : pop) {
    int S_index = ALG2::calculate_S(pi_index[0], pop[pop.size() - 1][0],
                                    pop[0][0], S_max, S_min);
    s.push_back(S_index);
  }
  return s;
...
vector<vector<int>> spatialDispersal(vector<int> s, const double sigma_min,
                                     const double sigma_max, ALG2::Sigma &sig) {
  vector<vector<int>> &pop = globalData.POP;
  ...
  for (int i = 0; i < pop.size(); ++i) {
    ...
    for (int j = 0; j < s[i]; j++) {
      ...
      Utils::sort_by_tot_processing_time(
          pi_R, globalData.total_processing_time); // 升序排序

      vector<vector<int>> e_2(n + 1, vector<int>(m + 1, 0));
      Utils::calculate_departure_time(e_2, 1, n - d, pi_new,
                                      processing_time); // 得到初始的e''

      vector<vector<int>> f_2(n + 1, vector(m + 2, 0));
      Utils::calculate_tail_time(f_2, n - d, 1, pi_new,
                                 processing_time); // 得到初始的f''
      ...
      for (int l = 0; l < d; l++) {
        ...
        pair<int, int> ans = Utils::neighbor_insertion(n - d + l, job, e_2, f_2,
                                                       processing_time);
        ...
        Utils::calculate_departure_time(e_2, best_index, n - d + l + 1, pi_new,
                                        processing_time); // 更新e''
        // 新f''的[best_index+1,n-d+l+1]用原f''的[best_index,n-d+l]
        copy(f_2.begin() + best_index, f_2.begin() + n - d + l + 1,
             f_2.begin() + best_index + 1);
        Utils::calculate_tail_time(f_2, best_index, 1, pi_new,
                                   processing_time); // 更新f''
        if (l == d - 1)
          pi_new[0] = bestmakespan;
      }
