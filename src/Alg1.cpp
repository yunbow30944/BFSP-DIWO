...
vector<int> insert_lambda(int lambda, vector<int> &pi_prime,
                          vector<vector<int>> d) {
  ...
  copy(pi_prime.begin() + 1, pi_prime.begin() + n - lambda + 1,
       order.begin() + 1);

  vector<vector<int>> e_2(n + 1, vector<int>(m + 1, 0));
  copy(d.begin() + 1, d.begin() + n - lambda + 1,
       e_2.begin() + 1); // 得到初始的e''
  vector<vector<int>> f_2(n + 1, vector<int>(m + 2, 0));
  Utils::calculate_tail_time(f_2, n - lambda, 1, order,
  ...
  for (int i = n - lambda + 1; i <= n; ++i) {
    ...
    pair<int, int> ans =
        Utils::neighbor_insertion(i - 1, job, e_2, f_2, processing_time);
    best_index = ans.first;
    bestmakespan = ans.second;
#ifdef IO_SHOW_PROCESSING_DATA
    cout << "best index: " << best_index << ", best makespan = " << bestmakespan
         << endl;
#endif
    order.insert(order.begin() + best_index, job); // 更新order
    Utils::calculate_departure_time(e_2, best_index, i, order,
                                    processing_time); // 更新e''
    // 新f''的[best_index+1,i]用原f''的[best_index,i-1]
    copy(f_2.begin() + best_index, f_2.begin() + i,
         f_2.begin() + best_index + 1);
    Utils::calculate_tail_time(f_2, best_index, 1, order,
                               processing_time); // 更新f''
    if (i == n)
...
vector<int> generate_one(const int lambda, const int k, vector<int> indice) {
  ...
  for (int i = 2; i <= n; i++) // 直到所有组件都被插入
  {
    ...
    for (int j = 1; j < indice.size(); ++j) // 尝试每个剩余的组件
    {
      ...
      d_now = Utils::calculate_v(v, i, order, d,
                                 processing_time); // 计算阻塞和空闲时间之和
      if (min_v > v)                               // 更新最优组件
      {
        best_indice = j;
        min_v = v;
