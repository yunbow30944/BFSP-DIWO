...
public:
  ...
  // 无参构造函数，只初始化超参数
  GlobalData()
      : n(0), m(0), lambda(25), x(5), P_max(10), S_MIN(0), S_MAX(7),
        SIGMA_MIN(5), SIGMA_MAX(10), pls(0.15), rho(90) {}

  // 有参构造函数，将算例信息保存
  GlobalData(const GlobalData &gDtmp)
      : n(gDtmp.n), m(gDtmp.m), processing_time(gDtmp.processing_time),
        total_processing_time(gDtmp.total_processing_time),
        indice(gDtmp.indice) {
    _dataMemoryPtr =
        new _dataMemory(n, m, processing_time, total_processing_time, indice);
  }
...
private:
  ...
  public:
    // 构造函数
    _dataMemory(const int &n, const int &m,
                const vector<vector<int>> &processing_time,
                const vector<int> &total_processing_time,
                const vector<int> &indice)
        : n(n), m(m), processing_time(processing_time),
          total_processing_time(total_processing_time), indice(indice) {}
  };
