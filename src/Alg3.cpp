...
vector<int> SRLS(vector<int> pi, vector<int> pi_r) {
  ...
  do {
    while (cnt < n) {
      ...
      Utils::calculate_departure_time(e_2, 1, n - 1, pi_prime,
                                      processing_time); // 得到初始的e''

      vector<vector<int>> f_2(n + 1, vector(m + 2, 0));
      Utils::calculate_tail_time(f_2, n - 1, 1, pi_prime,
                                 processing_time); // 得到初始的f''

      int bestfit = INT_MAX;
      int keypos = -1;
      pair<int, int> ans =
          Utils::neighbor_insertion(n - 1, pi_r[j], e_2, f_2, processing_time);
      keypos = ans.first;
      bestfit = ans.second;
