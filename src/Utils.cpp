...
void Utils::remove_non_improving_moves(
    const vector<vector<int>> &e, const vector<vector<int>> &f, const int c_max,
    vector<vector<int>> &V, const vector<int> order,
    const vector<vector<int>>
        processing_time) // 利用正反块去除掉没有意义的插入操作
{
...
void Utils::calculate_departure_time(
    vector<vector<int>> &e, const int begin, const int end,
    const vector<int> &order, const vector<vector<int>> &processing_time) {
  int m = processing_time[0].size() - 1;
  for (int i = begin; i <= end; ++i) {
    if (i == 1) {
      ...
      for (int j = 1; j <= m - 1; ++j)
        e[i][j] =
            max(e[i][j - 1] + processing_time[order[i]][j], e[i - 1][j + 1]);
    }
...
void Utils::calculate_tail_time(vector<vector<int>> &f, const int begin,
                                const int end, const vector<int> &order,
  ...
  for (int i = min(n, begin); i >= end; --i) //
  {
    if (i == n) {
      ...
      for (int j = m; j >= 2; --j)
        f[i][j] =
            max(f[i][j + 1] + processing_time[order[i]][j], f[i + 1][j - 1]);
    }
...
// 计算完工时间,old
int Utils::calculate(const vector<int> &order,
                     const vector<vector<int>> &processing_time) {
  ...
  for (int i = 2; i <= n; ++i) {
    ...
    for (int j = 1; j <= m - 1; ++j) {
      d[i][j] =
          max(d[i][j - 1] + processing_time[order[i]][j], d[i - 1][j + 1]);
    }
...
// 计算阻塞和空闲时间之和
vector<vector<int>>
Utils::calculate_v(int &v, const int i, const vector<int> &order,
                   vector<vector<int>> d,
                   const vector<vector<int>> &processing_time) {
  ...
  for (int j = 1; j <= m - 1; ++j) {
    d[i][j] = max(d[i][j - 1] + processing_time[order[i]][j], d[i - 1][j + 1]);
...
// 按总加工时间升序排列
void Utils::sort_by_tot_processing_time(
    vector<int> &pi_R, const vector<int> &total_processing_time) {
...
double Utils::ARPD(const int &R, const int &C_min, const vector<int> &C_i) {
  ...
  double ans =
      (static_cast<double>(100 * C_sum) / static_cast<double>(R * C_min)) -
      100.0;
  return ans;
}

// 返回位置、makespan
pair<int, int> Utils::neighbor_insertion(int length, int job,
                                         vector<vector<int>> e_2,
                                         vector<vector<int>> f_2,
  ...
  for (int q = 1; q <= length + 1; q++) {
    ...
    if (q == 1) {
      ...
      for (int j = 1; j <= m - 1; ++j)
        e_prime_q[j] =
            max(e_prime_q[j - 1] + processing_time[job][j], e_2[q - 1][j + 1]);
    }
