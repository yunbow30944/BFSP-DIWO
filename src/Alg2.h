...
public:
  ...
  Sigma(const vector<vector<int>> &pop) : pop(pop) { start = clock(); }
...
vector<vector<int>> spatialDispersal(vector<int> s, double sigma_min,
                                     double sigma_max, ALG2::Sigma &sig);
#endif // ALG2_H
