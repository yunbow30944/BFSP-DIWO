...
void singleFileProcess(const std::string &filePath, std::ofstream &fileOut) {
  ...
  while (std::getline(file, line)) {
    ...
    // 保存数据
    fileOut << filePath << "," << globalData.n << "," << globalData.m << ","
            << cnt << ",";
...
void getBestSeqAndSave() {
  if (!std::filesystem::exists(datasetDirPath) ||
      !std::filesystem::is_directory(datasetDirPath)) {
    throw std::domain_error("Invalid datasetDirPath!");
  }

  std::ofstream fileOut("../data/bestSequence.csv", std::ios::app);
  fileOut
      << "File path,Num of jobs,Num of machines,Dataset,Best sequence of all"
      << std::endl;
  // 遍历目录
  for (const auto &entry :
       std::filesystem::directory_iterator(datasetDirPath)) {
...
void loadMemory(const GlobalData &memory) {
  ...
  globalData.processing_time.assign(memory.n + 1,
...
void singleFileProcessWithARPD(
    const std::string &filePath, std::ofstream &fileOut,
  ...
  while (std::getline(file, line)) {
    ...
    std::string datasetName =
        to_string(globalData.n) + "*" + to_string(globalData.m);
    for (int i = 0; i < R; i++) {
      double ARPD_Result =
          run_single_ARPD_Calculate(1, C_min_map[datasetName][cnt]);
      fileOut << filePath << "," << globalData.n << "," << globalData.m << ","
              << cnt << "," << ARPD_Result << "," << globalData.bestmakespan
              << ",";
      for (int j = 1; j < globalData.best_seq.size(); j++)
        fileOut << globalData.best_seq[j] << " ";
      fileOut << "," << globalData.P_max << "," << globalData.S_MIN << ","
              << globalData.S_MAX << "," << globalData.SIGMA_MIN << ","
              << globalData.SIGMA_MAX << "," << globalData.pls << ","
              << globalData.rho << "\n";
    }
    cnt++;
  }
...
void getARPD_BestSeqAndSave(const int &R) {
  ...
  if (write_header) {
    fileOut << "File path,Num of jobs,Num of "
               "machines,Dataset,ARPD,Bestmakespan,Best sequence of all,"
  ...
  while (std::getline(CminFile, line)) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      iss >> trash >> cmin1 >> trash >> trash >> trash >> cmin2 >> trash >>
          trash >> trash >> cmin3 >> trash >> trash;
  ...
  // 遍历目录
  // fileOut << "File path,Num of jobs,Num of machines,Dataset,ARPD,Best
  // sequence of all" << std::endl;
  for (const auto &entry :
       std::filesystem::directory_iterator(datasetDirPath)) {
    if (std::filesystem::is_regular_file(entry.status())) {
      // 判断是否是文件
