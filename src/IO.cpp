...
namespace IO {
    ...
    void singleFileProcessWithARPD(const std::string &filePath, std::ofstream &fileOut,
                                   std::unordered_map<std::string, std::vector<int> > &C_min_map, const int &R) {
        std::ifstream file(filePath);
        if (!file.is_open())
     ...
     */
    void singleFileProcessWithARPD_SingleRun(const std::string &filePath, std::ofstream &fileOut,
